...
async function run() {
    ...
    console.log("Fetching ALL RSVPs (Kind 31925) for challenge " + challengeATag);
    const rsvps = await ndk.fetchEvents({ kinds: [31925], "#a": [challengeATag] });
    console.log("Found " + rsvps.size + " RSVPs.");
    for (const r of rsvps) {
        console.log("RSVP ID: " + r.id.substring(0,8) + " Author: " + r.pubkey + " Status: " + r.getMatchingTags("l")[0]?.[1]);
    }
    process.exit(0);
}
run();
