...
async function run() {
    ...
    if (challenge) {
        ...
        console.log("Challenge Window:");
        console.log("  Start: " + start + " (" + new Date(parseInt(start)*1000).toISOString() + ")");
        console.log("  End:   " + end + " (" + new Date(parseInt(end)*1000).toISOString() + ")");
    }

    console.log("\nFetching ride: " + rideId);
    const ride = await ndk.fetchEvent(rideId);
    if (ride) {
        console.log("Ride Created At: " + ride.created_at + " (" + new Date(ride.created_at*1000).toISOString() + ")");
